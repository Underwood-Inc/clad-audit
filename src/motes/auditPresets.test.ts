...
describe('auditPresets', () => {
  test('every preset rule id is built-in', () => {
  ...
  test('structure preset merges apps and import rules', () => {
  ...
  test('full preset runs all rules', () => {
    const rules = resolvePresetRules(presetById('full'));
    expect(rules).toHaveLength(ALL_CLAD_RULES.length);
  });

  test('filterFindingsByTierFocus keeps tier-tagged and path-matched findings', () => {
    const findings = [
