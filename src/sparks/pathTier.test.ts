...
describe('pathTier', () => {
  ...
  test('[FR-002] normalizes windows paths', () => {
    expect(normalizeRelativePath('src\\apps\\foo.ts')).toBe('src/apps/foo.ts');
  });

  test('[FR-002] resolves standard tier folders', () => {
  ...
  test('[FR-002] extraViewPaths are config-driven', () => {
  ...
  test('[FR-002] matches allowlist patterns', () => {
  ...
  test('[FR-002] parses static imports', () => {
  ...
  test('[FR-002] resolves relative import tier', () => {
    const tier = resolveImportTier('../recipes/coordinatorUi/paint.ts', 'src/molecules/a.ts', config);
    expect(tier).toBe('recipes');
  });

  test('[FR-002] glob ** matches nested paths', () => {
    const nestedPath = 'src/dist/foo';
    expect(globToRegExp('**/dist/**').test(nestedPath)).toBe(true);
  });

  test('[FR-002] countLines', () => {
    expect(countLines('a\nb\n')).toBe(3);
  });
});
